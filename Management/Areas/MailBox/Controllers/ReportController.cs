...
namespace Management.Areas.MailBox.Controllers
{
    ...
    public class ReportController : BaseController
    {
        ...
        /// <returns></returns>
        public IActionResult List(string strDate, string endDate, string strDate2, string endDate2, string caseclass, string dep, string replysource, int p = 1, int displayCount = 10)
        {
            var _dep = 0;
            ...
            pager.p = p;
            var lists = MailBoxService.GetReport(websiteId, strDate, endDate, strDate2, endDate2, _caseclass, _dep, replysource, ref pager);
            ListModels.defaultPager = pager;
            ListModels.ReportModel = lists;
            return View(ListModels);
        }

        public IActionResult ExcelReport(string strDate, string endDate, string strDate2, string endDate2, string caseclass, string dep, string replysource)
        {
            var _dep = 0;
            int.TryParse(dep, out _dep);
            int.TryParse(caseclass, out int _caseclass);
            var websiteId = UserData.WebSiteID;
            var fileName = $@"民意信箱統計報表{DateTime.Now.ToString("yyyyMMdd")}.xlsx";
            try
            {
                ...
                var list = MailBoxService.GetReport(websiteId, strDate, endDate, strDate2, endDate2, _caseclass, _dep, replysource);
                var filePath = $@"{path}\Temp\UserReport.xlsx";
                var Info = new List<string>() {
                $@"製表日期: {DateTime.Now.ToString("yyyy-MM-dd HH:mm")}",
