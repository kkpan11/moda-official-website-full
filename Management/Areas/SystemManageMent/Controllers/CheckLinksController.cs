    ...
    public class CheckLinksController : BaseController
    {
        ...
        public IActionResult CreateExportFile(List<CheckLinkModel.CheckingLink> links, string websiteid)
        {
            try
            {
                ...
                var fileName = $@"無效連結掃描{websiteid}{DateTime.Now.ToString("yyyyMMdd")}.xlsx";
                var path = _hostingEnvironment.WebRootPath;
                var excelDatas = new MemoryStream();

                var filePath = $@"{path}\Temp\CheckLinksReport.xlsx";
                var Info = new List<string>() {
                    $@"建表日期: {DateTime.Now.ToString("yyyy-MM-dd HH:mm")}",
                ...
                using (FileStream file = new FileStream($@"{path}\Temp\{fileName}", FileMode.Create, FileAccess.Write))
                {
                    excelDatas.WriteTo(file);
                }
        ...
        {
            var path = _hostingEnvironment.WebRootPath;
            var filePath = $@"{path}\Temp\{fileName.Replace("..","")}";
