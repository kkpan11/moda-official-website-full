    ...
    public class DepartmentManagementService
    {
        ...
        public static sysDepartmentModel Create(SysDepartment department, string webSiteID)
        {
            ...
            try
            {
                ...
                using (var db = new MODAContext())
                {
                    ...
                    ad = db.SysDepartment.FirstOrDefault(x => x.DepartmentID == department.DepartmentID.Trim() && x.IsEnable != "-99");
                    if (ad != null)
                    {
        ...
        public static sysDepartmentModel Edit(SysDepartment department)
        {
            ...
            try
            {
                using (var db = new MODAContext())
                {
                    var ad = db.SysDepartment.FirstOrDefault(x => x.DepartmentName == department.DepartmentName.Trim() && x.SysDepartmentSN != department.SysDepartmentSN && x.WebSiteId == department.WebSiteId && x.IsEnable != "-99");
                    if (ad != null)
                    {
                        sysDepartmentModel.check = false;
                        sysDepartmentModel.message = "單位名稱已存在";
                        return sysDepartmentModel;
                    }
                    if (department.SysDepartmentSN == department.MainSN)
                    {
                        ad = db.SysDepartment.FirstOrDefault(x => x.DepartmentID == department.DepartmentID.Trim() && x.SysDepartmentSN != department.SysDepartmentSN && x.MainSN == x.SysDepartmentSN && x.WebSiteId == department.WebSiteId && x.IsEnable != "-99");
                        if (ad != null)
                        {
                            sysDepartmentModel.check = false;
        ...
        public static sysDepartmentModel Delete(int SysDepartmentSN)
        {
            ...
            try
            {
                using (var db = new MODAContext())
                {
                    ...
                    {
                        depData.IsEnable = "-99";
                        depData.ProcessDate = DateTime.Now;
