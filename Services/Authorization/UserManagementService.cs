...
namespace Services.Authorization
{
    public class UserManagementService
    {
        ...
        public static sysUserModel Login(SysUser user , string aesKeyUp)
        {
            ...
            try
            {
                ...
                using (var db = new MODAContext())
                {
                    ...
                    #region 驗證機制
                    else if (ad.ErrLoginnum > 1 && ad.LastLoginDate.Value.AddMinutes(15) > DateTime.Now)
                    {
                        sysUserModel.check = false;
                    ...
                    {
                        if (ad.DisableDate.Value < DateTime.Now)
                        {
                            sysUserModel.check = false;
                    ...
                    if (sysUserModel.check)
                    {
                        ...
                        if (GroupData.Any(x => x.GroupName.Contains("系統管理")))
                        {
                            ...
                            sysUserModel.sysWebSites = sysWebSites;
        ...
        /// <param name="user"></param>
        /// <returns></returns>
        public static sysUserModel CreateUser(SysUser user)
        {
            sysUserModel sysUserModel = new sysUserModel();
            try
            {
                using (var db = new MODAContext())
                {
                    ...
                    user.p_w_d = AES.AesEncrypt(user.p_w_d, "MODA" + user.UserID.Trim());
                    var ad = db.SysUser.FirstOrDefault(x => x.UserID == user.UserID);
                    if (ad != null)
                    {
                        sysUserModel.check = false;
        ...
        public static sysUserModel EditUser(SysUser user)
        {
            ...
            try
            {
                using (var db = new MODAContext())
                {
                    ...
                    data.UserSatus = user.DisableDate < DateTime.Now ? "0" : user.UserSatus;
            ...
            try
            {
                using (var db = new MODAContext())
                {
                    ...
                    if (!string.IsNullOrEmpty(user.p_w_d))
                    {
                        ...
                        data.PwdLastUpdate = DateTime.Now;
            ...
            using (var db = new MODAContext())
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (cus.DisableDate.Value <= DateTime.Now)
                            {
                                sysUserModel.check = false;
            ...
            using (var db = new MODAContext())
            {
                try
                {
                    ...
                    var data = db.SysUser.FirstOrDefault(x => x.UserID == UserID
                           && x.p_w_d == key
                           && (x.DisableDate == null || x.DisableDate <= DateTime.Now)
                           && (type == 0 ? 1 == 1 : x.ProcessDate.Value.AddMinutes(10) >= DateTime.Now)
                         );
        ...
        public static sysUserModel UpdatePwd(SysUser user , string aeskey)
        {
            ...
            using (var db = new MODAContext())
            {
                try
                {
                    ...
                    uModel.ProcessDate = DateTime.Now;
                    uModel.PwdLastUpdate = DateTime.Now;
                    ...
                    else
                    {
                        ...
                        minpwdHistory.CreateDate = DateTime.Now;
