...
namespace Services.Static
{
    ...
    {
        public static string mainWebSiteID { get; set; }
        ...
        public static void Save(WebLevel webLevel, string webSiteDNS)
        {
            try
            {
                ...
                var StaticWebSiteID = webLevel.WebSiteID == mainWebSiteID ? "" : $@"/{webLevel.WebSiteID}";
                #region 連結處理
                ...
                if (!isUrlLink)
                {
                    ...
                    staticModel.CreatedDate = DateTime.Now;
                    staticModel.ProcessUserID = webLevel.ProcessUserID;
                    staticModel.ProcessDate = DateTime.Now;
            ...
            using (var db = new MODAContext())
            {
                try
                {
                    ...
                    if (StaticLink != null)
                    {
                        ...
                        {
                            parent.IsLive = "0";
                            parent.ProcessDate = DateTime.Now;
        ...
        public static List<StaticLink> GetStaticData()
        {
            try
            {
                Console.WriteLine("--str GetStaticData---");
                var allData = GetAllStaticLink();
                var TidyData = GetTidyData(allData);
                Console.WriteLine("--str GetStaticData---" + TidyData.Count().ToString());
                return TidyData;
            }
            catch (Exception ex)
            { 
        ...
        static List<StaticLink> GetTidyData(List<StaticLink> staticLinks)
        {
           ...
           (x.EndDate.HasValue ? x.EndDate.Value < DateTime.Now : false)
            ).ToList();
        ...
        public static void IsLiveSaveStatic(List<StaticLink> staticLinks, bool _chk)
        {
            ...
            {
                Console.WriteLine("IsLiveSaveStatic error ---  " + ex.ToString());
