        ...
        public static List<newsChildModel> GetChildNews(int key, string str, string end, string keyword, string depid, ref DefaultPager pager)
        {
            try
            {
                ...
                using (var db = new MODAContext())
                {
                    ...
                    list = list.Where(x => x.WebLevelSN == key && x.IsEnable == "1" && (x.StartDate <= DateTime.Now || x.StartDate == null) && (x.EndDate >= DateTime.Now || x.EndDate == null));
            ...
            try
            {
                using (var db = new MODAContext())
                {
                    NewsDetailModel.BasicData = db.WEBNews
                       ...
                       (ft == "0" ?(x.StartDate == null || x.StartDate <= DateTime.Now)&& (x.EndDate == null || x.EndDate >= DateTime.Now) :1==1)
                       );
                    ...
                    NewsDetailModel.Files = (from n in db.WEBNews.Where(x => x.WEBNewsSN == NewsSN)
                                             ...
                                             && (ft == "0" ? (n.StartDate == null || n.StartDate <= DateTime.Now) : 1 == 1)
                                             && (ft == "0" ? (n.EndDate == null || n.EndDate >= DateTime.Now) : 1 == 1)
                                             orderby r.SortOrder
                                             select f).ToList();
