...
namespace Utility.MailBox
{
    public class Api
    {
            ...
            using (HttpClient client = new HttpClient(setHttpNotSafeSSl()))
            {
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    try
                    {
                        ...
                        returnFileModel.SendDateTime = DateTime.Now;
                        return true;
                    }
                    catch 
                    {
        ...
        /// <param name="addNewCaseModel"></param>
        /// <returns></returns>
        public static bool AddNewCase(returnToekenModel returnToekenModel, AddNewCaseModel addNewCaseModel, List<returnFileModel> _returnFileModel, out returnAddNewCaseModel returnAddNewCaseModel)
        {
            var url = @$"{Url}/api/Case/AddNewCase";
            addNewCaseModel.LocationLongitude = "121.56354950781463";
            addNewCaseModel.LocationLatitude = "25.037418240062607";

            using (var client = new HttpClient(setHttpNotSafeSSl()))
            {
                var files = new string[0];
                if (_returnFileModel?.Count() > 0)
                {
                    files = _returnFileModel.SelectMany(x => x.FileNo).ToArray();
                }
                addNewCaseModel.Attachments = files;
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    try
                    {
                        ...
                        returnAddNewCaseModel.SendDateTime = DateTime.Now;
            ...
            using (var client = new HttpClient(setHttpNotSafeSSl()))
            {
                ...
                if (!string.IsNullOrEmpty(result))
                {
                    try
                    {
                        ...
                        returnSearchCaseModel.SendDateTime = DateTime.Now;
        ...
        public static List<FileMessage> DownloadAttachment(returnToekenModel returnToekenModel, List<ReplySaveFileName> replySaveFileNames, string CaseNo, string CompanyId)
        {
            ...
            foreach (var f in replySaveFileNames)
            {
                ...
                using (var client = new HttpClient(setHttpNotSafeSSl()))
                {
                    ...
                    httpContent.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/json");
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(returnToekenModel.token_type, returnToekenModel.access_token);
                    var result = client.PostAsync(url, httpContent).Result.Content.ReadAsByteArrayAsync().Result;
                    SaveFile.bytes = result;
                }
                #endregion
                #region 上傳
                using (var client = new HttpClient(setHttpNotSafeSSl()))
