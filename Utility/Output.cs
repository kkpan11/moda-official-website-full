...
namespace Utility
{
    public class Output
    {
        ...
        /// <param name="excelModel"></param>
        /// <param name="fileName"></param>
        public static MemoryStream SheetReport(string fileName, ExcelModel excelModel, List<ExcelDetailModel> DataList, List<ExcelDetailModel> CountList, List<string> Sheet, List<dynamic> WEBLevelData, string website, ExcelModel HelpOther = null)
        {
            try
            {
                int startSheet = 0;
                ...
                foreach (var s in Sheet)
                {
                    startSheet = x;
                    startRow = x;
                    var name = s == "zh-tw" ? "中文" : s == "en" ? "英文" : s;
                    if (x > 0)
                    {
                        Workbook.CreateSheet(website + "(" + name + ")");
                    }
                    else
                    {
                        Workbook.SetSheetName(startSheet, website + "(" + name + ")");
                    }
                    ISheet mySheet = Workbook.GetSheetAt(startSheet);
                    var endRow = 0;
                    var n = 0;
                    ...
                    {
                        case "統計":
                            endRow = startRow + (excelModel.Info == null ? 0 : excelModel.Info.Count()) + 1;
                            n = startRow + (excelModel.Info == null ? 0 : excelModel.Info.Count());
                            break;
                        case "代操作":
                            endRow = startRow + (HelpOther.Info == null ? 0 : HelpOther.Info.Count()) + 1;
                            n = startRow + (HelpOther.Info == null ? 0 : HelpOther.Info.Count());
                            break;
                        default:
                            endRow = startRow + (excelModel.Info == null ? 0 : excelModel.Info.Count()) + 1;
                            n = startRow + (excelModel.Info == null ? 0 : excelModel.Info.Count());
                    ...
                    switch (name)
                    {
                        case "統計":
                            ...
                            {
                                targetCell = targetRows.CreateCell(t);
                                if (excelModel.DetailTitle[t] == "完整路徑")
                                {
                            ...
                            #endregion
                            #region 報表資料
                            for (int d = 0; d < CountList.Count(); d++)
                            {
                                targetRows = mySheet.CreateRow(i);
                                for (int t = 0; t < excelModel.DetailTitle.Count(); t++)
                                {
                                    ...
                                    switch (t)
                                    {
                                        ...
                                        case 1: targetCell.SetCellValue(CountList[d].a); break;
                                        case 2: targetCell.SetCellValue(CountList[d].b); break;
                                        case 3: targetCell.SetCellValue(CountList[d].c); break;
                                        case 4: targetCell.SetCellValue(CountList[d].d); break;
                                        case 5: targetCell.SetCellValue(CountList[d].e); break;
                                        case 6: targetCell.SetCellValue(CountList[d].f); break;
                                        case 7: targetCell.SetCellValue(CountList[d].g); break;
                                        case 8: targetCell.SetCellValue(CountList[d].h); break;
                                        case 9: targetCell.SetCellValue(CountList[d].i); break;
                                        case 10: targetCell.SetCellValue(CountList[d].j); break;
                                    }
                                }
                                i++;
                            }
                        ...
                        case "代操作":
                            ...
                            #region 設定報表Detail標題
                            targetRows = mySheet.CreateRow(i);
                            for (int t = 0; t < HelpOther.DetailTitle.Count(); t++)
                            {
                                targetCell = targetRows.CreateCell(t);
                                mySheet.AutoSizeColumn((short)t);
                                targetCell.SetCellValue(HelpOther.DetailTitle[t]);
                                targetCell.CellStyle = style;
                            }
                            i++;
                            #endregion
                            for (int d = 0; d < HelpOther.ExcelDetails.Count(); d++)
                            {
                                targetRows = mySheet.CreateRow(i);
                                for (int t = 0; t < HelpOther.DetailTitle.Count(); t++)
                                {
                                    ...
                                    switch (t)
                                    {
                                        case 0: targetCell.SetCellValue(HelpOther.ExcelDetails[d].a); break;
                                        case 1: targetCell.SetCellValue(HelpOther.ExcelDetails[d].b); break;
                                        case 2: targetCell.SetCellValue(HelpOther.ExcelDetails[d].c); break;
                                        case 3: targetCell.SetCellValue(HelpOther.ExcelDetails[d].d); break;
                                    }
                                }
                                i++;
                            }
                            ...
                            var Module = new List<string>()
                        {
                            ...
                            foreach (var m in Module)
                            {
                                ...
                                {
                                    case "WEBNEWS":
                                        foreach (var data in WEBLevelData)
                                        {
                                            #region 標題
                                            targetRows = mySheet.CreateRow(i);
                                            targetCell = targetRows.CreateCell(0);
                                            targetCell.SetCellValue(data.Title);
                                            mySheet.AddMergedRegion(new NPOI.SS.Util.CellRangeAddress(i, i, 0, excelModel.DetailTitle.Count() - 1)); //合併儲存格
                                            targetCell.CellStyle = style0;
                                            ...
                                            #region 報表資料
                                            var DD = DataList.Where(x => x.l == data.WebLevelSN.ToString() && x.k == s).ToList();
                                    ...
                                    case "WEBLEVEL":
                                        ...
                                        #region 報表資料
                                        var LL = DataList.Where(x => x.m.ToUpper() == "WEBLEVEL" && x.k == s).ToList();

                                        for (int d = 0; d < LL.Count(); d++)
                                        {
                                            targetRows = mySheet.CreateRow(i);
                                            for (int t = 0; t < excelModel.DetailTitle.Count(); t++)
                                            {
                                                ...
                                                {
                                                    case 0: targetCell.SetCellValue((d + 1)); break;
                                                    case 1: targetCell.SetCellValue(LL[d].a); break;
                                                    case 2: targetCell.SetCellValue(LL[d].b); break;
                                                    case 3: targetCell.SetCellValue(LL[d].c); break;
                                                    case 4: targetCell.SetCellValue(LL[d].d); break;
                                                    case 5: targetCell.SetCellValue(LL[d].e); break;
                                                    case 6: targetCell.SetCellValue(LL[d].f); break;
                                                    case 7: targetCell.SetCellValue(LL[d].g); break;
                                                    case 8: targetCell.SetCellValue(LL[d].h); break;
                                                    case 9: targetCell.SetCellValue(LL[d].i); break;
                                                    case 10: targetCell.SetCellValue(LL[d].j); break;
                                                }
                                            }
                                            i++;
                                        }
                                    ...
                                    default:
                                        ...
                                        #endregion
                                        #region 報表資料
                                        var HH = DataList.Where(x => x.m.ToUpper() != "WEBLEVEL" && x.m.ToUpper() != "WEBNEWS" && x.k == s).ToList();
                                        for (int d = 0; d < HH.Count(); d++)
                                        {
                                            targetRows = mySheet.CreateRow(i);
                                            for (int t = 0; t < excelModel.DetailTitle.Count(); t++)
                                            {
                                                ...
                                                {
                                                    case 0: targetCell.SetCellValue((d + 1)); break;
                                                    case 1: targetCell.SetCellValue(HH[d].a); break;
                                                    case 2: targetCell.SetCellValue(HH[d].b); break;
                                                    case 3: targetCell.SetCellValue(HH[d].c); break;
                                                    case 4: targetCell.SetCellValue(HH[d].d); break;
                                                    case 5: targetCell.SetCellValue(HH[d].e); break;
                                                    case 6: targetCell.SetCellValue(HH[d].f); break;
                                                    case 7: targetCell.SetCellValue(HH[d].g); break;
                                                    case 8: targetCell.SetCellValue(HH[d].h); break;
                                                    case 9: targetCell.SetCellValue(HH[d].i); break;
                                                    case 10: targetCell.SetCellValue(HH[d].j); break;
                                                    case 11: targetCell.SetCellValue(HH[d].k); break;
                                                }
                                            }
                                            i++;
                                        }
